...
AppConfig bind_config(const hydra::ConfigNode &root) {
    ...
    cfg.model.activation = hydra::utils::expect_string(root, {"model", "activation"});

    cfg.trainer.batch_size = hydra::utils::expect_int(root, {"trainer", "batch_size"});
    cfg.trainer.max_epochs = hydra::utils::expect_int(root, {"trainer", "max_epochs"});

    cfg.logging.level = hydra::utils::expect_string(root, {"logging", "level"});

    cfg.experiment.name = hydra::utils::expect_string(root, {"experiment", "name"});
    cfg.experiment.run_dir = hydra::utils::expect_string(root, {"hydra", "run", "dir"});
...
int main(int argc, char **argv) try {
    ...
    if (!hydra::utils::has_node(config, {"experiment", "name"})) {
        hydra::assign_path(config, {"experiment", "name"}, hydra::make_string("cpp_example"), true);
    }

    fs::path run_dir_path = hydra::utils::write_hydra_outputs(config, cli.overrides);
    ...
    std::cout << "Logging level      : " << app.logging.level << '\n';
    std::cout << "hydra.run.dir      : " << app.experiment.run_dir << '\n';

    std::cout << "\n--- simulated training job ---\n";
    const int64_t dataset_size = 512;
    const int64_t steps_per_epoch = (dataset_size + app.trainer.batch_size - 1) /
                                    (app.trainer.batch_size > 0 ? app.trainer.batch_size : 1);
    for (int epoch = 1; epoch <= app.trainer.max_epochs && epoch <= 3; ++epoch) {
        std::cout << "[epoch " << epoch << "/" << app.trainer.max_epochs << "] running "
                  << steps_per_epoch << " steps (log level: " << app.logging.level << ")\n";
    }
    if (app.trainer.max_epochs > 3) {
        std::cout << "... (" << (app.trainer.max_epochs - 3) << " more epochs omitted) ...\n";
    }

    std::cout << "\n--- resolved config ---\n";
    hydra::utils::write_yaml(std::cout, config);
    std::cout << "\n(Hydra outputs written under " << run_dir_path << "/.hydra)\n";
