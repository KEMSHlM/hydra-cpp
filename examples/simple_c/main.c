...
static void ensure_experiment_name(hydra_config_t *cfg) {
    ...
    if (!hydra_config_has(cfg, "experiment.name")) {
        exit_on_error("set default experiment",
                      hydra_config_apply_override(cfg, "+experiment.name=\"c_example\"", &err),
                      err);
    }
}
...
static AppConfig load_app_config(hydra_config_t *cfg) {
    ...
    AppConfig config = {
        ...
        .logging = {
            .level = hydra_config_expect_string(cfg, "logging.level"),
        },
        .experiment = {
...
static void free_app_config(AppConfig *config) {
    ...
    hydra_string_free(config->logging.level);
...
static void simulate_training_job(const AppConfig *config) {
    ...
    for (int64_t epoch = 1;
         epoch <= config->trainer.max_epochs && epoch <= 3;
         ++epoch) {
        printf("[epoch %" PRId64 "/%" PRId64 "] running %" PRId64 " steps (log level: %s)\n",
               epoch,
               config->trainer.max_epochs,
               steps_per_epoch,
               config->logging.level);
...
int main(int argc, char **argv) {
    ...
    printf("\n--- resolved config ---\n");
    exit_on_error("render config",
                  hydra_config_stream_yaml(cfg, stdout, &err),
                  err);
